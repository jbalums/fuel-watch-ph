  ...
  useEffect(() => {
    if (activePosition) {
      setViewportCenter((current) =>
        coordinatesMatch(current, activePosition) ? current : activePosition,
      );
    }
  }, [activePosition]);
  ...
  const resolveAddress = useCallback(
    async (coordinates: CoordinatePair) => {
      ...
      try {
        ...
        if (!formattedAddress) {
          throw new Error("No address was returned for this location");
        }
        ...
        <div className="flex items-start gap-2">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Tap a station marker to report an existing station, or click
              anywhere else to pin a new station location.
            </p>
          </div>
        </div>
        ...
        {selectedStation ? (
          <>
            <p className="font-medium text-foreground">
              Reporting update for existing station: {selectedStation.name}
            </p>
          ...
          <>
            ...
            <p>{selectedAddress ?? buildPinnedLocationLabel(selectedPosition)}</p>
          </>
        ) : (
          <p>Select an existing station or click the map to pin a new one.</p>
        )}
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Add `VITE_GOOGLE_MAPS_API_KEY` to use the report location picker.
            </p>
          </div>
        </div>
      </div>
