...
ALTER TABLE public.gas_stations
  ADD COLUMN IF NOT EXISTS previous_prices jsonb;
...
SET previous_prices = public.empty_fuel_price_map() || COALESCE(previous_prices, '{}'::jsonb),
    price_trends = public.calculate_station_price_trends_from_previous_prices(
      COALESCE(prices, public.empty_fuel_price_map()),
      fuel_type,
      price_per_liter,
      public.empty_fuel_price_map() || COALESCE(previous_prices, '{}'::jsonb)
    )
WHERE previous_prices IS NULL
   OR NOT (previous_prices ? 'Premium Diesel');

ALTER TABLE public.gas_stations
  ALTER COLUMN previous_prices SET NOT NULL;
