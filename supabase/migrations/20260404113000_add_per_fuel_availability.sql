...
CREATE OR REPLACE FUNCTION public.is_sellable_station_status(_status text)
RETURNS boolean
LANGUAGE sql
IMMUTABLE
SET search_path = public
AS $$
  SELECT _status IN ('Available', 'Low');
$$;
...
CREATE OR REPLACE FUNCTION public.derive_legacy_station_status(
  _fuel_availability jsonb,
  _fuel_type text,
  _fallback_status text DEFAULT 'Out'
)
RETURNS text
LANGUAGE plpgsql
IMMUTABLE
...
DECLARE
  _resolved_status text;
BEGIN
  _resolved_status := public.resolve_station_fuel_availability(
    _fuel_availability,
    _fuel_type,
    NULL
  );
...
BEGIN
  ...
  IF _status = 'Out' AND _fuel_type IN ('Unleaded', 'Premium', 'Diesel', 'Premium Diesel') THEN
...
ALTER TABLE public.gas_stations
  ADD COLUMN IF NOT EXISTS fuel_availability jsonb;
...
WHERE fuel_availability IS NULL
   OR NOT (fuel_availability ? 'Premium Diesel');
...
ALTER TABLE public.gas_stations
  ALTER COLUMN fuel_availability SET NOT NULL;

ALTER TABLE public.fuel_reports
  ALTER COLUMN fuel_availability SET NOT NULL;

DROP FUNCTION IF EXISTS public.update_managed_station(uuid, text, text, text, jsonb);
DROP FUNCTION IF EXISTS public.update_managed_station(uuid, text, text, jsonb, jsonb);

CREATE FUNCTION public.update_managed_station(
  _station_id uuid,
  _address text,
  _fuel_type text,
  _prices jsonb,
...
BEGIN
  ...
  IF _report_selected_fuel_type IS NULL THEN
    _report_selected_fuel_type := CASE
      WHEN _report.fuel_type IN ('Unleaded', 'Premium', 'Diesel', 'Premium Diesel') THEN _report.fuel_type
      ELSE 'Diesel'
    END;
  ...
  IF _report.station_id IS NOT NULL THEN
    ...
    IF _selected_fuel_type IS NULL THEN
      _selected_fuel_type := CASE
        WHEN _station.fuel_type IN ('Unleaded', 'Premium', 'Diesel', 'Premium Diesel') THEN _station.fuel_type
